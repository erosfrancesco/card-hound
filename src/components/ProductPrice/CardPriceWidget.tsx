  ...
  if (loading)
    return (
      <div className="text-center py-7 text-slate-500">
        Loading CardTrader Zero prices...
      </div>
    );
  if (error)
    return (
  ...
  return (
    <div className="p-4 border border-slate-200 rounded-lg">
      ...
      {stats.lowestPrice !== null && (
        <div className="mb-3">
          <p className="text-sm">
            <strong>Lowest CT Zero Price:</strong> {stats.lowestPrice}{" "}
            {stats.currency}
          </p>
          <p className="text-sm">
      ...
      <ul className="list-disc pl-5 m-0">
        {products.map((item) => (
          <li key={item.id} className="text-sm">
            <strong>
              {(item.price.cents / 100).toFixed(2)} {item.price.currency}
            </strong>
            {" - "}
            ...
            Lang:{" "}
            {(
              item.properties_hash.language ||
              item.properties_hash.mtg_language ||
              "N/A"
            ).toUpperCase()}
            {" | "}
